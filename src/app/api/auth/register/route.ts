...
export async function POST(request: Request) {
  try {
    ...
    // Authoritative validation: runs no matter how the request reaches the API
    // (form, curl, Postman, scripts...), unlike the browser-only form checks.
    const validationError = validateRegistration({ name, email, password });
    if (validationError) {
      return Response.json(
        { data: null, code: 400, message: validationError },
        { status: 400 }
      );
    }
